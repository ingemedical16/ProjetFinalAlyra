...
contract MultiSender is  OwnedUpgradeabilityStorage, Claimable {
    ...
    function multisendToken(address[] memory _contributors, uint256[] memory _balances) public hasFee payable {
            ...
            ERC20 btuToken = ERC20(tokenAddress());
            uint8 i = 0;
            for (i; i < _contributors.length; i++) {
                btuToken.transferFrom(msg.sender, _contributors[i], _balances[i]);
                total += _balances[i];
            }
    ...
    function claimTokens(address _token) public onlyOwner {
        ...
        ERC20 btuToken = ERC20(_token);
        uint256 balance = btuToken.balanceOf(address(this));
        btuToken.transfer(owner(), balance);
